...
function generateMedicalCardPdf(profile, encounterData) {
  return new Promise((resolve, reject) => {
    try {
      ...
      // ========================================
      doc.fontSize(28).fillColor('#00ACC1').text('Dr.Dori', { align: 'center' });
      doc.moveDown(0.3);

      // 이름 + Patient Summary
      doc.fontSize(16).fillColor('#000000').text(`Name: ${profile.name || 'N/A'}`, { align: 'center' });
      doc.fontSize(14).fillColor('#000000').text('Patient Summary', { align: 'center' });
      doc.moveDown(0.5);

      // 상태 배지 (is_emergency에 따라 색상 변경)
      if (encounterData.is_emergency) {
        doc.fillColor('#FFFFFF')
           .rect(doc.page.width - 200, 100, 150, 35)
           .fillAndStroke('#FF0000', '#FF0000');
        doc.fillColor('#FFFFFF').fontSize(14).text('[WARNING]', doc.page.width - 190, 110);
      } else {
        doc.fillColor('#FFFFFF')
           .rect(doc.page.width - 200, 100, 150, 35)
           .fillAndStroke('#00AA00', '#00AA00');
        doc.fillColor('#FFFFFF').fontSize(12).text('[NORMAL]', doc.page.width - 180, 107);
        doc.fontSize(9).text('(No Red Flags detected)', doc.page.width - 185, 121);
      }
      ...
      doc.fontSize(12)
        ...
        .text(`Gender/Age: ${profile.gender || 'N/A'} / ${calculateAge(profile.birth_date)}`, 50, currentY1 + 18)
        .text(`Language: ${profile.language || 'English (Primary)'}`, 50, currentY1 + 36);
      ...
      // ========================================
      doc.fontSize(15).fillColor('#000000').text('Chief Complaint: ', { continued: true, underline: true })
        .fontSize(13).fillColor('#000000').text(`${encounterData.cc_kor} (${encounterData.cc_eng})`, { underline: false });
      doc.moveDown(0.5);
      ...
      // ========================================
      doc.fontSize(14).fillColor('#000000').text('History of Present Illness', { underline: true });
      doc.moveDown(0.5);
      ...
      doc.fontSize(12).fillColor('#000000')
        .text(`Pain Score: ${encounterData.pain_score || 'N/A'} / 10`, 50, doc.y);
      doc.moveDown(0.5);
      doc.fontSize(11).fillColor('#555555')
        .text(`Pain Score: ${encounterData.pain_score || 'N/A'}`, 50, doc.y);
      doc.moveDown(2);

      // ========================================
      // 6. Medical History & Alerts
      // ========================================
      doc.fontSize(16).fillColor('#000000').text('Medical History & Alerts', { underline: true });
      doc.moveDown(0.5);
      ...
      // Allergies - 빨간색 + 굵게
      doc.fontSize(13).fillColor('#FF0000')
      ...
      // Suggested Department - 빨간색 + 굵게
      doc.fontSize(13).fillColor('#FF0000')
        .text(`Suggested Dept: `, { continued: true })
        .text(`"${encounterData.suggested_dept_kor || 'N/A'} (${encounterData.suggested_dept_eng || 'Internal Medicine'})"`, { continued: false });
      doc.moveDown(0.5);
